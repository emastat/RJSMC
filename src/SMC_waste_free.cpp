...
List SMC_waste_free_cpp( const IntegerVector& Yvec,
  ...
  for(int j=0; j<n_UI ; j++ ){
    ...
    if(sum(Tvec>start_point & Tvec<end_point)==0){
      ...
      // and being t_star taken from B_last then min(B_last) < t_star is always true
      ...
      // index of particles ending with a non-empty segment
      IntegerVector non_empty_particle_index_vec = seq_particle[B_last_non_empty_seg_logical] ;

      // index of particles ending with an empty segment
      IntegerVector empty_particle_index_vec = seq_particle[B_last_empty_seg_logical] ;


      // update particles with non-empty last segment


      if(non_empty_particle_index_vec.length()>0){

        // create the weights for sampling the recycled particles from the non empty particles vector
        NumericVector non_empty_weights = weight_vec[non_empty_particle_index_vec] ;

        NumericVector normalized_non_empty_weights = non_empty_weights / sum(non_empty_weights);

        // sample the recycled particles (talk to Geir about the weights)

        IntegerVector recycled_non_empty_particle_index ;
        NumericVector t_star_non_empty_seg;

        // if there are less particles to be sampled than recycled_particles, take all of them
        if(non_empty_particle_index_vec.length() < recycled_particles ){

          recycled_non_empty_particle_index  =  clone(non_empty_particle_index_vec) ;

        }else{

          recycled_non_empty_particle_index  =  Rcpp::sample(non_empty_particle_index_vec, recycled_particles, false, normalized_non_empty_weights )  ;

        }

        // get last_B for the recycled particles
        t_star_non_empty_seg =  B_last[recycled_non_empty_particle_index] ;

        //assign each non empty particle to a recycled particle
        IntegerVector group_index = rep_len(recycled_non_empty_particle_index, non_empty_particle_index_vec.length() ) ;

        //store each particle in the correct subgroup (according to which recycled particle they have been assigned)

        List particle_list(recycled_non_empty_particle_index.length()) ;

        for(int i =0 ; i<particle_list.length(); i++){

          particle_list[i] = non_empty_particle_index_vec[group_index == recycled_non_empty_particle_index[i]] ;

          }

        // this loop will be replaced  with the parallel loop
        for (int k=0 ; k<particle_list.length(); k++){
      ...
      if(empty_particle_index_vec.length()>0){
        ...
        // create the weights for sampling one particle from the empty particles
        NumericVector empty_weights = weight_vec[empty_particle_index_vec] ;

        NumericVector normalized_empty_weights = empty_weights / sum(empty_weights);

        // sample 1 particle (only for now, talk to Geir)

        int sampled_empty_particle_index  = as<int>( Rcpp::sample(empty_particle_index_vec, 1,false,normalized_empty_weights ) ) ;

        // get last_B for the sampled particle

        double  t_star_empty_seg =  B_last[sampled_empty_particle_index] ;
