...
# Set origin date for plotting
origin_date <- as.Date("2024-01-01")
plot_V <- plot(out_SMC,
               origin = origin_date,
               pl = "V",
               truth = list(B = true_Bvec, cl = true_Vvec),
               observations = list(Tvec = Tvec, Yvec = Yvec))
print(plot_V)

cat("\n  Creating plot for Z (segment length)...\n")
plot_Z <- plot(out_SMC,
               origin = origin_date,
               pl = "Z",
               truth = list(B = true_Bvec, cl = true_Zvec),
               observations = list(Tvec = Tvec, Yvec = Yvec))
print(plot_Z)

cat("\n  Creating plot for Q (message rate)...\n")
plot_Q <- plot(out_SMC,
               origin = origin_date,
               pl = "Q",
               truth = list(B = true_Bvec, cl = true_Qvec),
               observations = list(Tvec = Tvec, Yvec = Yvec))
print(plot_Q)

cat("\n  Creating plot for F (empty segment length)...\n")
plot_F <- plot(out_SMC,
               origin = origin_date,
               pl = "F",
               truth = list(B = true_Bvec, cl = true_Fvec),
               observations = list(Tvec = Tvec, Yvec = Yvec))
print(plot_F)
...
cat("True Values Summary:\n")
cat("  - Total segments:", length(true_Bvec) - 1, "\n")
cat("  - Non-empty segments:", sum(true_Vvec != 0), "\n")
cat("  - Empty segments:", sum(true_Vvec == 0), "\n")
cat("  - Unique V states:", length(unique(true_Vvec[true_Vvec != 0])), "\n")
cat("  - Unique Z states:", length(unique(true_Zvec[true_Zvec != 0])), "\n")
cat("  - Unique Q states:", length(unique(true_Qvec[true_Qvec != 0])), "\n")
cat("  - Unique F states:", length(unique(true_Fvec[true_Fvec != 0])), "\n\n")

cat("Posterior Summary:\n")
cat("  - Points evaluated:", length(out_SMC@points_container), "\n")
cat("  - Time range: [", min(out_SMC@points_container), ",", 
    max(out_SMC@points_container), "] hours\n\n")

cat("Verification Instructions:\n")
cat("  1. Check that the colored rectangles (true values) align with\n")
cat("     high probability regions in the area plots\n")
cat("  2. Verify that breakpoints in true values correspond to\n")
cat("     changes in posterior distributions\n")
cat("  3. Ensure observations (dots) are distributed according to\n")
cat("     the true segment structure\n")
cat("  4. Compare posterior probabilities across different state variables\n\n")
